...
test('useExtractReferences extracts file references from text content', () => {
    const part: TextUIPart = {
        type: 'text',
        text: 'The auth flow is implemented in @file:{github.com/nkbud/sourcebot::auth.ts} and uses sessions @file:{github.com/nkbud/sourcebot::auth.ts:45-60}.'
    }
    ...
    expect(result.current[0]).toMatchObject({
        ...
        path: 'auth.ts',
        id: getFileReferenceId({ repo: 'github.com/nkbud/sourcebot', path: 'auth.ts' }),
        type: 'file',
    });
